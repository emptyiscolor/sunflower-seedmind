...
function generate_cov() {
  ...
  while IFS= read -r line; do
    ...
    if [ -d "$OSSFUZZ_DIR/build/corpus/seedgen/$project_name" ] && [ -f "$OSSFUZZ_DIR/build/corpus/seedgen/$project_name/$binary_name.json" ] ; then
      echo "Generating code coverage: $project_name"
      #   python infra/helper.py build_fuzzers --sanitizer=coverage $project_name
      echo "python infra/helper.py coverage --fuzz-target=$binary_name --corpus-dir="$OSSFUZZ_DIR/build/corpus/seedgen/$project_name" --no-serve $project_name "
      timeout 30m python infra/helper.py coverage --fuzz-target=$binary_name --corpus-dir="$OSSFUZZ_DIR/build/corpus/seedgen/$project_name" --no-serve $project_name 
      if [ $? -eq 124 ]; then
