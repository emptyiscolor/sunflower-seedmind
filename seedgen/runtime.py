...
from . import seedd_pb2
from . import seedd_pb2_grpc
...
class SeedDRuntime:
    ...
    def wait_until_ready(self):
        logger.info("Waiting for SeedD service to be ready...")
        self.ready_event.wait()
        logger.info("SeedD service is ready.")

    def _connect(self):
        while True:
            logger.info(
                f"Connecting to the gRPC server at {
            ...
            try:
                ...
                self.channel = grpc.insecure_channel(
                    self.grpc_server_address, options=options)
                ...
                if (
                    ...
                    logger.warning(
                        "Health check failed, service is not healthy.")
                    self.channel.close()
                    self._log_and_sleep(
                        f"Sleeping for {
                            RETRY_INTERVAL} seconds before retrying...",
                        RETRY_INTERVAL,
                    )
    ...
    def _grpc_call(self, request, method_name: str):
        ...
        try:
            logger.info(
                f"Calling {method_name} method with request: {request}")
    ...
    def locate(self, harness_binary: str, function_name: str) -> Optional[tuple]:
        locate_request = seedd_pb2.LocateRequest(
    ...
    def view(self, filename: str, line: int) -> Optional[str]:
        view_request = seedd_pb2.ViewRequest(filename=filename, line=line)
    ...
    def share(self, filename: str) -> Optional[str]:
        share_request = seedd_pb2.ShareRequest(filename=filename)
    ...
    def run(self, harness_binary: str, seeds_path: List[str]) -> Optional[str]:
        run_request = seedd_pb2.RunRequest(
    ...
    def export_calls(self, harness_binary: str, seeds_path: List[str]) -> Optional[str]:
        export_calls_request = seedd_pb2.ExportCallsRequest(
            harness_binary=harness_binary, seeds_path=seeds_path
        )
